    ...
    : QWidget(parent), m_logPanel(logPanel)
{
    ...
    connect(m_checkSelectedBtn, &QPushButton::clicked, this, &SyncPage::onCheckSelectedFromList);
    ...
    connect(m_worker, &SyncWorker::repoBranchInfo, this, &SyncPage::onRepoBranchInfo);
...
void SyncPage::onCheckSelectedFromList()
{
...
void SyncPage::onRepoBranchInfo(QString path, QString branchInfo)
{
...
void SyncPage::onRevertClean()
{
    ...
    if (selected.isEmpty()) {
        QMessageBox::information(this, QStringLiteral("Revert && Clean"), QStringLiteral("Please select at least one repository or submodule first."));
        return;
    }
    ...
    box.setText(QStringLiteral("This will discard ALL local changes (git reset --hard + git clean -fd) "
                               "in %1 selected repositories/submodules.\n\n"
    ...
    m_logPanel->appendInfo(QStringLiteral("Starting revert & clean for %1 selected item(s)...").arg(selected.size()));
...
void SyncPage::onRevertCleanFinished()
{
    showProblemsSummary(QStringLiteral("Revert && Clean Finished"),
                         QStringLiteral("All selected repositories/submodules were reverted and cleaned successfully."));
}
...
void SyncPage::onPull()
{
    const QStringList selected = selectedPaths();
    if (selected.isEmpty()) {
        QMessageBox::information(this, QStringLiteral("Pull"), QStringLiteral("Please select at least one repository or submodule first."));
        return;
    }
    ...
    box.setText(QStringLiteral("Pull the latest changes in %1 selected repositories/submodules?").arg(selected.size()));
    box.setStandardButtons(QMessageBox::Yes | QMessageBox::No);
    box.setDefaultButton(QMessageBox::No);
    if (box.exec() != QMessageBox::Yes)
        return;
    ...
    beginOperation(selected);
    m_logPanel->appendInfo(QStringLiteral("Starting pull for %1 selected item(s)...").arg(selected.size()));
...
void SyncPage::onPullFinished()
{
    showProblemsSummary(QStringLiteral("Pull Finished"),
                         QStringLiteral("All selected repositories/submodules were pulled successfully. No conflicts."));
}
...
void SyncPage::onSwitchBranch()
{
    ...
    box.setText(QStringLiteral("Switch %1 selected repositories/submodules to branch '%2'?").arg(selected.size()).arg(branch));
    box.setStandardButtons(QMessageBox::Yes | QMessageBox::No);
    box.setDefaultButton(QMessageBox::No);
    if (box.exec() != QMessageBox::Yes)
        return;
    ...
    beginOperation(selected);
    m_logPanel->appendInfo(QStringLiteral("Switching %1 selected item(s) to branch '%2'...").arg(selected.size()).arg(branch));
...
void SyncPage::onSwitchFinished()
{
    showProblemsSummary(QStringLiteral("Branch Switch Finished"),
                         QStringLiteral("All selected repositories/submodules were switched successfully."));
}
